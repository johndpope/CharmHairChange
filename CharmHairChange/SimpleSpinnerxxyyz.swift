...
public class SimpleSpinnerxxyyz {
    ...
    func xyyxcreateSpinner(abbx: String?, abby: String?, insideOfView:UIView, titleArray:[String], textAlignment:NSTextAlignment, dropTextAlignment:UIControl.ContentHorizontalAlignment, callback:@escaping (_ position:Int) -> Void) {
        DispatchQueue.main.async {
            ...
            spinnerBtn.addTargetClosure{ _ in
                ...
                if (self.isEnable) {
                    if (self.spinnerTitleStringArray.count > 0) {
                        self.isEnable = false
                        self.xyyxspinnerScrollViewDisplay(abbx: nil, abby: nil, spinnerView: self.spinnerSourceView!)
                    }
                }
    ...
    func xyyxremoveSpinner(abbx: String?, abby: String?) {
        if (spinnerSourceView != nil) {
            spinnerSourceView?.removeFromSuperview()
        }
    }
    
    func xyyxspinnerScrollViewDisplay(abbx: String?, abby: String?, spinnerView:UIView) {
        
        let absFrame = xyyxgetAbsoluteFame(abbx: nil, abby: nil, sourceView: spinnerView)
        let outermostView = xyyxgetOutermostView(abbx: nil, abby: nil, sourceView: spinnerView)
        ...
        spinnerCancelBtn?.addTargetClosure(closure: { (sender) in
            self.xyyxspinnerScrollViewCancel(abbx: nil, abby: nil)
        })
        ...
        itemBtn.addTargetClosure{ _ in
            ...
            self.xyyxspinnerScrollBtnClick(position: 0)
        ...
        if (spinnerTitleStringArray.count > 1) {
            for j in 2...spinnerTitleStringArray.count {
                ...
                itemBtn2.addTargetClosure{ _ in
                    ...
                    self.xyyxspinnerScrollBtnClick(position: j-1)
    ...
    func xyyxspinnerScrollViewCancel(abbx: String?, abby: String?) {
        isEnable = true
    ...
    func xyyxspinnerScrollBtnClick(position:Int) {
        xyyxsetSelection(abbx: nil, abby: nil, position: position) {
            self.xyyxspinnerScrollViewCancel(abbx: nil, abby: nil)
        }
    }
    
    func xyyxsetSelection(abbx: String?, abby: String?, position:Int, completion:(() -> Void)?) {
    ...
    func xyyxgetAbsoluteFame(abbx: String?, abby: String?, sourceView:UIView) -> CGRect {
